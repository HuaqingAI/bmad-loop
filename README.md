...
| `BMAD_LOOP_STATE_DIR`         | directory path                                 | Overrides the user-scoped **state root** — the out-of-tree home of per-run control-plane state, keyed `<root>/<project>/<run-id>/`. Used as the root itself, so nothing is appended to it. Unset ⇒ `$XDG_STATE_HOME/bmad-loop` when that names an absolute path, else `~/.local/state/bmad-loop`; on Windows `%LOCALAPPDATA%\bmad-loop\state`, else `%USERPROFILE%\AppData\Local\bmad-loop\state`. Set this when none of those is derivable or writable (a home on a network share, a locked-down service account). |
