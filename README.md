...
- **`commit_message_template`** — when set, the message used for story/bundle commits (`{story_key}` / `{run_id}` / `{story_title}` substituted; the title comes from the spec's `title:` frontmatter).
