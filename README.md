...
bmad-loop init --local-hooks --no-skills --cli codex  # lightweight setup in each clone/checkout
...
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --------------------------- | ------------------------------------------------------------- |
| `bmad-loop init`                                         | Install the bundled `bmad-loop-*` skills, the hook relay, `.bmad-loop/policy.toml`, and a gitignore for generated run/archive/cache/relay state, no-spec fallback results, and policy.toml itself (policy is per-machine — see the CHANGELOG migration note for repos initialized before this). `--cli <profile>` (repeatable) targets specific agents; `--no-skills` / `--force-skills` control skill copying; `--local-hooks` treats the selected CLI hook configs as checkout-local and pairs with `--no-skills` for lightweight per-clone initialization.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `bmad-loop run`                                          | Drive the dev → review → verify → commit loop. `--namespace PREFIX`, `--epic N`, `--story KEY`, `--max-stories N`, `--dry-run`. `--spec <folder>` forces **stories mode** (folder+id dispatch off `<folder>/stories.yaml`), overriding `[stories].source`; `--story` then filters by story id.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `bmad-loop sweep`                                        | Triage + execute open `deferred-work.md` entries. `--no-prompt`, `--decisions-only`, `--max-bundles N`, `--repeat`, `--max-cycles N`, `--dry-run`. `--archive [--before DATE]` instead moves closed ledger entries to `deferred-work-archive.md`, leaving id-preserving stubs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |     | `bmad-loop resume <run-id>` | Continue a run paused at a gate, escalation, or interruption. |
...
- **Sprint mode (default).** Stories come from `sprint-status.yaml` (written by `bmad-sprint-planning` from your PRD/epics). The loop walks the board by `ready-for-dev` status, keyed by story ref (`1-2-account-mgmt`). Boards with parallel delivery lanes may prefix every key, for example `L0-epic-2` / `L0-2-1-account-mgmt` and `L8B-epic-2` / `L8B-2-1-segments`. Select one with `run --namespace L0 --epic 2`, or set `[stories] namespace = "L0"` in the checkout-local policy. Matching is case-insensitive, dispatched keys preserve their original spelling, and namespaced specs are discovered under `implementation-artifacts/<namespace>/` before the legacy root. This is what the rest of this README describes.
...
uv tool install "bmad-loop[tui] @ git+https://github.com/HuaqingAI/bmad-loop.git@v0.9.1"
...
**Easiest — let the setup skill do it.** Re-running `/bmad-loop-setup` (or `/bmad-loop-setup upgrade`) on an already-installed project resolves the custom `huaqing-bmad-loop` source from BMAD's manifest, reinstalls the tool from that exact source, and re-lays the per-project skills with `--force-skills` — then reports the source and before → after version.
...
```bash
# 1. reinstall from the fork explicitly. --force also switches a machine that
#    previously had the official tool installed; --reinstall refreshes Git/cache data.
uv tool install --force --reinstall "bmad-loop[tui] @ git+https://github.com/HuaqingAI/bmad-loop.git"
#    to pin a tag, append the ref to this same fork URL:
#    uv tool install --force --reinstall "bmad-loop[tui] @ git+https://github.com/HuaqingAI/bmad-loop.git@v0.9.1"
...
**Via the BMAD-method installer.** Add this fork as a custom source; do not select the official `bmad-loop` module. The fork registers internally as `huaqing-bmad-loop`, while its CLI and skill names remain unchanged:
...
The installer copies the bundled `bmad-loop-*` skills into your project (but not the orchestrator tool), alongside the upstream dev primitive the orchestrator drives. Finish setup with `/bmad-loop-setup`, which reads the custom source from BMAD's manifest, installs the tool from that same source, asks which coding CLIs to drive, registers their hooks (`init` skips the already-present skills), and runs the preflight:
