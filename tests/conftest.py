...
def fault_metadata_probe(monkeypatch, target: Path, probe: str) -> None:
    ...
    — with ``stat`` the documented exception, see its own paragraph below.

    ``probe`` is one of ``exists`` / ``is_file`` / ``is_symlink`` / ``stat`` — one
    probe at a time, which does NOT make one guard-per-probe ablatable: a caller
    may well take all four inside a single ``try``, where one ``except`` covers the
    lot. What it buys is coverage of each ENTRY PATH into that one guard — each
    probe is reached only after the ones before it answered a particular way, so a
    row per probe proves every reachable arm is inside the guard rather than only
    the first.
    ...
    ``stat`` is the exception to that whole paragraph, and it is why DW-221 moved
    the repair/write ledger reader onto it: ``Path.stat`` suppresses NOTHING on
    any interpreter — CPython's pathlib docs name it as the probe that reports
    the error rather than answering False, which is exactly what ``is_file()``
    stopped doing in 3.14 when its body became ``os.path.isfile``. So a ``stat``
    fault injected here is not a simulation of an older runtime the way the other
