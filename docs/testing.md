...
- **The detector itself gets executable coverage.** Every detector added since this bar was set
  ...
  family, refusal-def, isolation-call, real-mux group mark, generic-tmux completion wall, and
  stories-E2E reap deadline: the source detectors (`_scan_source`, `_scan_session_walls`,
  `_scan_reap_deadlines`) are split so probe fixtures run the same code path as the real
  scan, with a must-flag row per claimed access form and a must-stay-silent row per
...
- **The guard's boundary is `src/bmad_loop`,** enforced structurally by its scan root rather
  ...
  and stories reap-deadline guards, whose subject is the suite's own scheduling and waits.
