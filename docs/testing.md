...
- **The detector itself gets executable coverage.** Every detector added since this bar was set
  ...
  family, refusal-def, isolation-call, real-mux group mark, and generic-tmux completion wall:
  the source detectors (`_scan_source`, `_scan_session_walls`) are split so probe fixtures run
...
- **The guard's boundary is `src/bmad_loop`,** enforced structurally by its scan root rather
  than by an allowlist. The deliberate exceptions are the real-mux grouping and
  completion-wall guards, whose subject is the suite's own scheduling and waits. There
  is no production artifact to scan, so they are rooted at `tests/` and live in
